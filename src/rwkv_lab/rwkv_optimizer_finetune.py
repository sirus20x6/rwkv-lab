...
def refuse_ungated_attempt_baseline(
    ...
    fresh attempt, the resume checkpoint's step for a replacement one. This
    trainer's pre-mutation crossing names ``step + 1``, and
    ``WorkerControlRuntime.optimizer_step`` refuses any step at or below the
    baseline, so an attempt that restored some other step can never reach a
    legal first mutation -- it would stall at the first crossing with nothing
    to explain why. Refusing here names the restored step instead.
    ...
    This route cannot arm the step-zero eval gate -- its authority profile
    declares no ``eval_examples`` output port, so no composition may publish
    one -- but the guard is written against the runtime's answer rather than
    against that fact, so it starts refusing the moment the port is added.
    """
    ...
    if step != baseline:
        raise ValueError(
            "RWKV optimizer finetuning resumed at a step the controller does "
            f"not gate: {step} != {baseline}"
        )
    ...
    if worker_controls.step_zero_eval_gate_satisfied:
        ...
        return
    raise ValueError(
        "RWKV optimizer finetuning cannot mutate: the controller requires "
        f"durable attempt-baseline evidence at step {baseline} and none is "
...
def train(
    ...
    *,
    ...
    def cross_mutation_boundary(next_step: int) -> None:
        ...
        A boundary that refuses -- a cancellation, or an unsatisfied
        attempt-baseline eval gate -- leaves the sentinel disarmed, so the
        mutation it was meant to guard fails rather than proceeding. The
        sentinel's pre-hook is process-global, so an update path that never
        reaches this function also fails closed instead of mutating unguarded.
        """
