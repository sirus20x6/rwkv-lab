"""Closed TrainVM configuration for raw-pixel Vision-RWKV distillation."""

from __future__ import annotations

import math
from dataclasses import dataclass


def _path(value: object, label: str) -> str:
    if not isinstance(value, str) or not value or "\x00" in value:
        raise ValueError(f"{label} must be a nonempty path string")
    return value


def _integer(value: object, label: str, minimum: int, maximum: int) -> int:
    if (
        not isinstance(value, int)
        or isinstance(value, bool)
        or not minimum <= value <= maximum
    ):
        raise ValueError(f"{label} must be an integer in [{minimum}, {maximum}]")
    return value


def _finite(value: object, label: str, minimum: float, maximum: float) -> float:
    if (
        isinstance(value, bool)
        or not isinstance(value, (int, float))
        or not math.isfinite(float(value))
        or not minimum <= float(value) <= maximum
    ):
        raise ValueError(f"{label} must be finite in [{minimum}, {maximum}]")
    return float(value)


@dataclass(frozen=True, slots=True)
class VisionRWKVStudentConfig:
    baseline_checkpoint: str
    compressor_checkpoint: str
    native_head_checkpoint: str
    train_manifest: str
    eval_manifest: str
    moon_cache: str
    fusion_cache: str
    moonvit_checkpoint: str
    siglip2_model: str
    dinov2_model: str
    sam_model: str
    vocab: str
    output_dir: str
    steps: int = 10_000
    batch_size: int = 1
    workers: int = 8
    learning_rate: float = 1.0e-4
    weight_decay: float = 0.05
    max_gradient_norm: float = 1.0
    caption_weight: float = 0.1
    teacher_reconstruction_weight: float = 0.25
    latent_relational_weight: float = 0.2
    teacher_relational_weight: float = 0.2
    variance_weight: float = 0.05
    covariance_weight: float = 0.01
    diversity_weight: float = 0.05
    eval_every: int = 250
    eval_examples: int = 32
    checkpoint_every: int = 250
    seed: int = 20260716
    image_size: int = 512
    grid_size: int = 16
    hidden_size: int = 2048
    layers: int = 26
    head_size: int = 64
    ffn_hidden: int = 7168
    checkpoint_blocks: bool = True
    device: str = "cuda"

    def __post_init__(self) -> None:
        for label in (
            "baseline_checkpoint",
            "compressor_checkpoint",
            "native_head_checkpoint",
            "train_manifest",
            "eval_manifest",
            "moon_cache",
            "fusion_cache",
            "moonvit_checkpoint",
            "siglip2_model",
            "dinov2_model",
            "sam_model",
            "vocab",
            "output_dir",
        ):
            _path(getattr(self, label), label)
        for label, minimum in (
            ("steps", 1),
            ("batch_size", 1),
            ("workers", 0),
            ("eval_every", 1),
            ("eval_examples", 1),
            ("checkpoint_every", 1),
            ("seed", 0),
            ("image_size", 32),
            ("grid_size", 1),
            ("hidden_size", 1),
            ("layers", 1),
            ("head_size", 1),
            ("ffn_hidden", 1),
        ):
            _integer(getattr(self, label), label, minimum, 1_000_000_000)
        for label, minimum, maximum in (
            ("learning_rate", 1.0e-12, 1.0),
            ("weight_decay", 0.0, 10.0),
            ("max_gradient_norm", 1.0e-12, 1.0e12),
            ("caption_weight", 0.0, 1.0e6),
            ("teacher_reconstruction_weight", 0.0, 1.0e6),
            ("latent_relational_weight", 0.0, 1.0e6),
            ("teacher_relational_weight", 0.0, 1.0e6),
            ("variance_weight", 0.0, 1.0e6),
            ("covariance_weight", 0.0, 1.0e6),
            ("diversity_weight", 0.0, 1.0e6),
        ):
            _finite(getattr(self, label), label, minimum, maximum)
        if self.eval_every > self.steps or self.checkpoint_every > self.steps:
            raise ValueError("eval/checkpoint cadence cannot exceed steps")
        if self.image_size % self.grid_size:
            raise ValueError("image_size must be divisible by grid_size")
        if self.hidden_size % self.head_size:
            raise ValueError("hidden_size must be divisible by head_size")
        if not isinstance(self.checkpoint_blocks, bool):
            raise TypeError("checkpoint_blocks must be boolean")
        if self.device != "cuda":
            raise ValueError("vision student v1 requires device cuda")


__all__ = ["VisionRWKVStudentConfig"]
