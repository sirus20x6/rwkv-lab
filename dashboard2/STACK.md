...
- **Procs:** `process.Processes()` → filter `Cmdline()` against the training-script allowlist
  (`train_mla.py, convert_train.py, convert_distill_progressive.py, distill_consolidate.py, fit_variant.py,
  convert_stack.py, train_mla_engram.py`); derive run name from `--out-dir`/`--out` arg, runtime from
