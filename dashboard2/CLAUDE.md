...
FastAPI + Chart.js v1). **Do not modify anything outside `dashboard2/`.** v1 stays running on its own port;
v2 coexists.
...
```bash
go -C /workspace/git/moe-mla/dashboard2 run ./cmd/trainboard      # dev
go -C /workspace/git/moe-mla/dashboard2 build -o trainboard ./cmd/trainboard
go -C /workspace/git/moe-mla/dashboard2 vet ./...
```
...
- Reads `/workspace/git/moe-mla/runs/` (the same logs v1 reads). SQLite file lives at
  `dashboard2/trainboard.db` (gitignore-able; rebuilt from JSONL on demand).
...
- **Avoid editing files outside this folder.** Training-script instrumentation lives as *copies* in
  `instrumented/` (duplicate-then-modify), keeping the real `../train_mla.py` pristine. **Exception (settled
  2026-06-30):** `../convert_train.py` is now the *canonical* trainer — its run-control hooks (SIGUSR1/SIGINT)
  are native and `sysmon.VerifyTrainingPID` recognizes it as signal-capable by name, so its `instrumented/`
  copy was reconciled in and retired to `../legacy/`. Do NOT re-create an `instrumented/convert_train.py`;
  edit the root copy. The dashboard launches root scripts by `RepoRoot + basename` (`control.go`).
- Port the proven bits of v1 rather than reinventing: liveness windows (healthy<300 s / stalling<900 s),
  `_finite` NaN→null sanitization, safetensors metadata-only param counting (`../dashboard/architecture.py`).
