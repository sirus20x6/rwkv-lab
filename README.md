...
> Paths default to the author's local layout (`/workspace/git/moe-mla/...`); override via the flags each script exposes (`--help`). Every training lever defaults **off** — at default flags the trainers reproduce the plain baseline. See [`TRAINING_LEVERS.md`](TRAINING_LEVERS.md).
