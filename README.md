...
- [CART: Context-Anchored Recurrent Transformer](https://arxiv.org/abs/2606.01495) — contractive LTI gate on the carried loop state (`out = σ(g)⊙out + inc`, spectral radius <1 → provable fixed-point convergence, damps deep-loop drift) → [`looped_rwkv.py`](looped_rwkv.py) (`--loop-cart-anchor`)
