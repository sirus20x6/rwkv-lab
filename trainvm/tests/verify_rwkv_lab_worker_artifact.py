...
def main() -> int:
    ...
    # The six MageFlow operations deliberately share one identical sealed
    # Python closure. Every other adapter currently owns a distinct closure.
    if len(grouped) != len(adapters) - 5 or not any(
        members == list(adapters[:6]) for members in grouped.values()
    ):
