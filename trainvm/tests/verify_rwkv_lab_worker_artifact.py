...
def main() -> int:
    ...
    # The five MageFlow operations deliberately share one identical sealed
    # Python closure. Every other adapter currently owns a distinct closure.
    if len(grouped) != len(adapters) - 4 or not any(
        members == list(adapters[:5]) for members in grouped.values()
    ):
