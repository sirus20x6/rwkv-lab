...
def main() -> int:
    ...
    # The seven MageFlow operations deliberately share one identical sealed
    # Python closure. Every other adapter currently owns a distinct closure.
    if len(grouped) != len(adapters) - 6 or not any(
        members == list(adapters[:7]) for members in grouped.values()
    ):
