...
constexpr std::string_view kProcessExitSchemaV5 = R"sql(
CREATE TABLE process_exits (
  launch_id TEXT PRIMARY KEY REFERENCES process_spawns(launch_id),
  exit_request_id TEXT NOT NULL UNIQUE,
  request_digest TEXT NOT NULL UNIQUE,
  spawn_receipt_digest TEXT NOT NULL UNIQUE,
  receipt_digest TEXT NOT NULL UNIQUE,
  canonical_request_json TEXT NOT NULL,
  canonical_receipt_json TEXT NOT NULL,
  record_sequence INTEGER NOT NULL UNIQUE CHECK(record_sequence > 0),
...
CREATE TRIGGER process_exits_no_update BEFORE UPDATE ON process_exits
BEGIN SELECT RAISE(ABORT, 'process exit receipts are immutable'); END;
CREATE TRIGGER process_exits_no_delete BEFORE DELETE ON process_exits
BEGIN SELECT RAISE(ABORT, 'process exit receipts are immutable'); END;
)sql";

// A restarted daemon is not the worker's parent and cannot truthfully invent
// waitid status. Version 6 therefore records a separate terminal receipt for
// exact pidfd/proc identity observations while leaving every v5 exit byte and
// projection untouched.
constexpr std::string_view kProcessRecoveryExitSchemaV6 = R"sql(
CREATE TABLE process_recovery_exits (
  launch_id TEXT PRIMARY KEY REFERENCES process_spawns(launch_id),
  recovery_exit_request_id TEXT NOT NULL UNIQUE,
  request_digest TEXT NOT NULL UNIQUE,
  spawn_receipt_digest TEXT NOT NULL UNIQUE,
  receipt_digest TEXT NOT NULL UNIQUE,
  canonical_request_json TEXT NOT NULL,
...
CREATE TRIGGER process_recovery_exits_no_update
BEFORE UPDATE ON process_recovery_exits
BEGIN SELECT RAISE(ABORT, 'process recovery exit receipts are immutable'); END;
CREATE TRIGGER process_recovery_exits_no_delete
BEFORE DELETE ON process_recovery_exits
BEGIN SELECT RAISE(ABORT, 'process recovery exit receipts are immutable'); END;
)sql";
...
bool valid_device_program_name(std::string_view value) {
  return value.size() == 15U && value.starts_with("tvmdev_") &&
         std::ranges::all_of(value.substr(7U), [](char character) {
           return (character >= '0' && character <= '9') ||
                  (character >= 'a' && character <= 'f');
         });
...
bool valid_device_policy_intent_binding(
    const HostDevicePolicyIntentBinding& binding) {
  return valid_digest(binding.policy_digest) &&
         valid_digest(binding.image_digest) &&
         valid_device_program_name(binding.program_name) &&
         binding.program_name ==
             "tvmdev_" + binding.image_digest.substr(7U, 8U);
}

bool valid_device_policy_installation_binding(
    const HostDevicePolicyInstallationBinding& binding) {
  return valid_digest(binding.policy_digest) &&
         valid_digest(binding.image_digest) && binding.program_id > 0U &&
         binding.program_type > 0U && binding.program_tag.size() == 16U &&
         std::ranges::all_of(binding.program_tag, [](char character) {
           return (character >= '0' && character <= '9') ||
                  (character >= 'a' && character <= 'f');
         }) &&
         valid_device_program_name(binding.program_name) &&
...
nlohmann::json process_launch_request_digest_json(
  ...
  nlohmann::json value{{"api_version", request.api_version},
...
nlohmann::json process_exit_request_digest_json(
  ...
  return {{"api_version", request.api_version},
          ...
          {"host_pid", request.host_pid},
          {"process_starttime_ticks", request.process_starttime_ticks},
          {"wait_code", request.wait_code},
          {"wait_status", request.wait_status},
          {"cgroup_path", request.cgroup_path},
          {"cgroup_device", request.cgroup_device},
          {"cgroup_inode", request.cgroup_inode},
          {"cgroup_empty", request.cgroup_empty},
          {"accelerator_contexts_empty", request.accelerator_contexts_empty},
          {"context_audit_digest", request.context_audit_digest}};
}

nlohmann::json process_exit_receipt_digest_json(
    const HostProcessExitReceipt& receipt) {
  return {{"api_version", receipt.api_version},
          {"request", host_process_exit_request_json(receipt.request)},
          {"host_id", receipt.host_id},
          {"boot_id", receipt.boot_id},
          {"broker_epoch", receipt.broker_epoch},
          {"observed_boottime_ns", receipt.observed_boottime_ns},
...
nlohmann::json process_recovery_exit_request_digest_json(
    const HostProcessRecoveryExitRequest& request) {
  return {{"api_version", request.api_version},
          {"recovery_exit_request_id", request.recovery_exit_request_id},
          {"launch_id", request.launch_id},
          {"spawn_receipt_digest", request.spawn_receipt_digest},
          {"host_pid", request.host_pid},
          {"process_starttime_ticks", request.process_starttime_ticks},
          {"observation", enum_to_string(request.observation)},
          ...
          {"accelerator_contexts_empty",
           request.accelerator_contexts_empty},
          {"context_audit_digest", request.context_audit_digest}};
}

nlohmann::json process_recovery_exit_receipt_digest_json(
    const HostProcessRecoveryExitReceipt& receipt) {
  return {{"api_version", receipt.api_version},
          {"request",
           host_process_recovery_exit_request_json(receipt.request)},
          {"host_id", receipt.host_id},
          {"boot_id", receipt.boot_id},
          {"broker_epoch", receipt.broker_epoch},
          {"observed_boottime_ns", receipt.observed_boottime_ns},
...
const SchemaSnapshot& canonical_schema_v5() {
  static const SchemaSnapshot snapshot = [] {
    ...
    try {
      ...
      execute(database, kProcessAuthoritySchemaV4,
              "canonical process-authority schema creation failed");
      execute(database, kProcessExitSchemaV5,
              "canonical process-exit schema creation failed");
      auto value = schema_snapshot(database);
      sqlite3_close(database);
      return value;
    } catch (...) {
...
const SchemaSnapshot& canonical_schema_v6() {
  static const SchemaSnapshot snapshot = [] {
    ...
    try {
      ...
      execute(database, kProcessExitSchemaV5,
              "canonical process-exit schema creation failed");
      execute(database, kProcessRecoveryExitSchemaV6,
              "canonical process-recovery-exit schema creation failed");
      auto value = schema_snapshot(database);
      sqlite3_close(database);
      return value;
    } catch (...) {
...
struct SQLiteHostLedger::Implementation final {
  ...
  bool verify_unlocked(std::string* reason,
                       ...
                       unsigned int schema_version = 6U) const {
    ...
    try {
      ...
      if ((schema_version == 1U &&
          ...
          (schema_version == 5U &&
           schema_snapshot(database) != canonical_schema_v5()) ||
          (schema_version == 6U &&
           schema_snapshot(database) != canonical_schema_v6()) ||
          (schema_version != 1U && schema_version != 2U &&
           schema_version != 3U && schema_version != 4U &&
           schema_version != 5U && schema_version != 6U)) {
        ...
        if (schema_version >= 3U) {
          ...
          if (schema_version >= 4U) {
            ...
            std::map<std::string,
                ...
                spawn_evidence;
            std::map<std::string,
                     std::pair<HostProcessExitReceipt, ProcessEvidence>>
                exit_evidence;
            std::map<
                std::string,
                std::pair<HostProcessRecoveryExitReceipt, ProcessEvidence>>
                recovery_exit_evidence;
            if (schema_version >= 5U) {
              Statement exit_extension(database, R"sql(
                SELECT schema_version FROM ledger_schema_extensions
                WHERE feature='process_exit'
              )sql");
              if (sqlite3_step(exit_extension.get()) != SQLITE_ROW ||
                  sqlite3_column_int64(exit_extension.get(), 0) != 5 ||
                  sqlite3_step(exit_extension.get()) != SQLITE_DONE) {
                return fail("process-exit schema extension marker is invalid");
              }
            }
            if (schema_version >= 6U) {
              Statement recovery_exit_extension(database, R"sql(
                SELECT schema_version FROM ledger_schema_extensions
                WHERE feature='process_recovery_exit'
              )sql");
              if (sqlite3_step(recovery_exit_extension.get()) != SQLITE_ROW ||
                  sqlite3_column_int64(recovery_exit_extension.get(), 0) != 6 ||
                  sqlite3_step(recovery_exit_extension.get()) != SQLITE_DONE) {
                return fail(
                    "process-recovery-exit schema extension marker is invalid");
              }
            }
            ...
            while ((status = sqlite3_step(process_records.get())) == SQLITE_ROW) {
              ...
              } else if (type == "process.exited" && schema_version >= 5U) {
                const nlohmann::json& payload = record.at("payload");
                const std::string payload_version =
                    payload.value("api_version", std::string{});
                if (payload_version == kHostProcessExitReceiptApiVersion) {
                  const HostProcessExitReceipt receipt =
                      host_process_exit_receipt_from_json(payload);
                  const auto spawn =
                      spawn_evidence.find(receipt.request.launch_id);
                  if (receipt.previous_process_receipt_digest !=
                          previous_process_receipt ||
                      spawn == spawn_evidence.end() ||
                      receipt.request.spawn_receipt_digest !=
                          spawn->second.first.receipt_digest ||
                      recovery_exit_evidence.contains(
                          receipt.request.launch_id) ||
                      record.value("record_id", std::string{}) !=
                          "process-exit:" + receipt.request.exit_request_id ||
                      record.value("subject_id", std::string{}) !=
                          receipt.request.launch_id ||
                      record.value("host_id", std::string{}) !=
                          receipt.host_id ||
                      record.value("boot_id", std::string{}) !=
                          receipt.boot_id ||
                      record.value("broker_epoch", std::string{}) !=
                      ...
                      !exit_evidence
                           .emplace(receipt.request.launch_id,
                                    std::pair{receipt, evidence})
                           .second) {
                    return fail("process exit evidence is malformed");
                  }
                } else if (
                    schema_version >= 6U &&
                    payload_version ==
                        kHostProcessRecoveryExitReceiptApiVersion) {
                  const HostProcessRecoveryExitReceipt receipt =
                      host_process_recovery_exit_receipt_from_json(payload);
                  const auto spawn =
                      spawn_evidence.find(receipt.request.launch_id);
                  if (receipt.previous_process_receipt_digest !=
                          previous_process_receipt ||
                      ...
                      receipt.request.spawn_receipt_digest !=
                          spawn->second.first.receipt_digest ||
                      exit_evidence.contains(receipt.request.launch_id) ||
                      record.value("record_id", std::string{}) !=
                          "process-recovery-exit:" +
                              receipt.request.recovery_exit_request_id ||
                      record.value("subject_id", std::string{}) !=
                          receipt.request.launch_id ||
                      record.value("host_id", std::string{}) !=
                          receipt.host_id ||
                      ...
                      !recovery_exit_evidence
                           ...
                           .second) {
                    return fail(
                        "process recovery exit evidence is malformed");
                  }
                } else {
                  return fail("process terminal evidence version is invalid");
                }
              } else {
            ...
            if (schema_version >= 5U) {
              Statement exits(database, R"sql(
                SELECT launch_id, exit_request_id, request_digest,
                       spawn_receipt_digest, receipt_digest,
                       canonical_request_json, canonical_receipt_json,
                       record_sequence, record_receipt_digest, record_chain_hash
                FROM process_exits ORDER BY launch_id
              )sql");
              std::size_t exit_count = 0U;
              while ((status = sqlite3_step(exits.get())) == SQLITE_ROW) {
                ++exit_count;
                const std::string launch_id = column_text(exits.get(), 0);
                const auto found = exit_evidence.find(launch_id);
                if (found == exit_evidence.end()) {
                  return fail("process exit has no authority record");
                }
                const auto& [receipt, evidence] = found->second;
                if (column_text(exits.get(), 1) !=
                        receipt.request.exit_request_id ||
                    column_text(exits.get(), 2) !=
                        receipt.request.canonical_request_digest ||
                    column_text(exits.get(), 3) !=
                        receipt.request.spawn_receipt_digest ||
                    column_text(exits.get(), 4) != receipt.receipt_digest ||
                    column_text(exits.get(), 5) !=
                        host_process_exit_request_json(receipt.request).dump() ||
                    column_text(exits.get(), 6) !=
                        host_process_exit_receipt_json(receipt).dump() ||
                    static_cast<std::uint64_t>(
                        sqlite3_column_int64(exits.get(), 7)) !=
                        evidence.sequence ||
                    column_text(exits.get(), 8) != evidence.record_receipt ||
                    column_text(exits.get(), 9) != evidence.chain_hash) {
                  return fail("process exit projection is not canonical");
                }
              }
              if (status != SQLITE_DONE || exit_count != exit_evidence.size()) {
                return fail("process exit evidence is not closed");
              }
              if (schema_version >= 6U) {
                Statement recovery_exits(database, R"sql(
                  SELECT launch_id, recovery_exit_request_id, request_digest,
                         spawn_receipt_digest, receipt_digest,
                         canonical_request_json, canonical_receipt_json,
                         record_sequence, record_receipt_digest,
                         record_chain_hash
                  FROM process_recovery_exits ORDER BY launch_id
                )sql");
                std::size_t recovery_exit_count = 0U;
                while ((status = sqlite3_step(recovery_exits.get())) ==
                       SQLITE_ROW) {
                  ++recovery_exit_count;
                  const std::string launch_id =
                      column_text(recovery_exits.get(), 0);
                  const auto found = recovery_exit_evidence.find(launch_id);
                  if (found == recovery_exit_evidence.end() ||
                      exit_evidence.contains(launch_id)) {
                    return fail(
                        "process recovery exit has no exclusive authority record");
                  }
                  const auto& [receipt, evidence] = found->second;
                  if (column_text(recovery_exits.get(), 1) !=
                          receipt.request.recovery_exit_request_id ||
                      column_text(recovery_exits.get(), 2) !=
                          receipt.request.canonical_request_digest ||
                      column_text(recovery_exits.get(), 3) !=
                          receipt.request.spawn_receipt_digest ||
                      column_text(recovery_exits.get(), 4) !=
                          receipt.receipt_digest ||
                      column_text(recovery_exits.get(), 5) !=
                          host_process_recovery_exit_request_json(
                              receipt.request)
                              .dump() ||
                      column_text(recovery_exits.get(), 6) !=
                          host_process_recovery_exit_receipt_json(receipt)
                              .dump() ||
                      static_cast<std::uint64_t>(sqlite3_column_int64(
                          recovery_exits.get(), 7)) != evidence.sequence ||
                      column_text(recovery_exits.get(), 8) !=
                          evidence.record_receipt ||
                      column_text(recovery_exits.get(), 9) !=
                          evidence.chain_hash) {
                    return fail(
                        "process recovery exit projection is not canonical");
                  }
                }
                if (status != SQLITE_DONE ||
                    recovery_exit_count != recovery_exit_evidence.size()) {
                  return fail(
                      "process recovery exit evidence is not closed");
                }
                Statement unsafe_release(database, R"sql(
                  SELECT 1 FROM process_launch_intents AS intent
                  ...
                  WHERE allocation.status='released' AND
                        terminal.launch_id IS NULL AND
                        recovered.launch_id IS NULL LIMIT 1
                )sql");
                if (sqlite3_step(unsafe_release.get()) == SQLITE_ROW) {
                  return fail(
                      "released allocation lacks terminal process authority");
                }
              } else {
                if (!recovery_exit_evidence.empty()) {
                  return fail("v5 process chain contains v6 recovery exit");
                }
                Statement unsafe_release(database, R"sql(
                  SELECT 1 FROM process_launch_intents AS intent
                  JOIN process_spawns AS spawn USING(launch_id)
                  ...
                  WHERE allocation.status='released'
                    AND terminal.launch_id IS NULL LIMIT 1
                )sql");
                if (sqlite3_step(unsafe_release.get()) == SQLITE_ROW) {
                  return fail(
                      "released allocation lacks terminal process authority");
            ...
            } else if (!exit_evidence.empty() ||
                       !recovery_exit_evidence.empty()) {
              return fail("v4 process chain contains v5 exit evidence");
            }
          }
        }
...
SQLiteHostLedger::SQLiteHostLedger(
  ...
  if (!initialized) {
    ...
    execute(implementation_->database, kProcessExitSchemaV5,
            "could not create process-exit schema v5");
    execute(implementation_->database, kProcessRecoveryExitSchemaV6,
            "could not create process-recovery-exit schema v6");
    execute(implementation_->database,
            "PRAGMA application_id=0x5456484c; PRAGMA user_version=6;",
    ...
    execute(implementation_->database,
            ...
            "INSERT INTO ledger_schema_extensions(feature, schema_version) "
            "VALUES('process_exit', 5);"
            "INSERT INTO ledger_schema_extensions(feature, schema_version) "
            "VALUES('process_recovery_exit', 6);",
    ...
    if (opened_schema == canonical_schema_v1() && user_version == 1) {
      ...
      migration.commit();
               ...
               (opened_schema != canonical_schema_v5() || user_version != 5) &&
               (opened_schema != canonical_schema_v6() || user_version != 6)) {
      throw HostLedgerError(
    ...
    const std::int64_t post_process_schema_version = [&] {
      Statement current_version(implementation_->database,
                                "PRAGMA user_version");
      if (sqlite3_step(current_version.get()) != SQLITE_ROW)
        throw HostLedgerError("could not reread process schema version");
      return sqlite3_column_int64(current_version.get(), 0);
    }();
    if (post_process_schema_version == 4) {
      Transaction migration(implementation_->database);
      std::string migration_reason;
      if (!implementation_->verify_unlocked(&migration_reason, false, 4U)) {
        throw HostLedgerError("refusing process-exit migration: " +
                              migration_reason);
      }
      execute(implementation_->database, kProcessExitSchemaV5,
              "could not create process-exit schema v5");
      execute(implementation_->database,
              "INSERT INTO ledger_schema_extensions(feature, schema_version) "
              "VALUES('process_exit', 5); PRAGMA user_version=5;",
              "could not mark process-exit schema v5");
      if (!implementation_->verify_unlocked(&migration_reason, false, 5U)) {
        throw HostLedgerError("process-exit migration verification failed: " +
                              migration_reason);
      }
      migration.commit();
    }
    const std::int64_t post_exit_schema_version = [&] {
      Statement current_version(implementation_->database,
                                "PRAGMA user_version");
      if (sqlite3_step(current_version.get()) != SQLITE_ROW)
        throw HostLedgerError("could not reread process-exit schema version");
      return sqlite3_column_int64(current_version.get(), 0);
    }();
    if (post_exit_schema_version == 5) {
      Transaction migration(implementation_->database);
      std::string migration_reason;
      if (!implementation_->verify_unlocked(&migration_reason, false, 5U)) {
        throw HostLedgerError("refusing process-recovery-exit migration: " +
                              migration_reason);
      }
      execute(implementation_->database, kProcessRecoveryExitSchemaV6,
              "could not create process-recovery-exit schema v6");
      execute(implementation_->database,
              "INSERT INTO ledger_schema_extensions(feature, schema_version) "
              "VALUES('process_recovery_exit', 6); PRAGMA user_version=6;",
              "could not mark process-recovery-exit schema v6");
      if (!implementation_->verify_unlocked(&migration_reason, false, 6U)) {
        throw HostLedgerError(
            "process-recovery-exit migration verification failed: " +
            migration_reason);
      }
      migration.commit();
    }
...
BundleReleaseResult SQLiteHostLedger::release_bundle(
  ...
  Statement process_terminal(implementation_->database, R"sql(
    SELECT EXISTS(
             SELECT 1 FROM process_launch_intents AS intent
             JOIN process_spawns AS spawn USING(launch_id)
             WHERE intent.allocation_id=?
           ),
           EXISTS(
             SELECT 1 FROM process_launch_intents AS intent
             JOIN process_spawns AS spawn USING(launch_id)
             LEFT JOIN process_exits AS terminal USING(launch_id)
             LEFT JOIN process_recovery_exits AS recovered USING(launch_id)
             WHERE intent.allocation_id=? AND
                   terminal.launch_id IS NULL AND
                   recovered.launch_id IS NULL
           )
  )sql");
  bind_text(process_terminal.get(), 1, request.allocation_id);
  bind_text(process_terminal.get(), 2, request.allocation_id);
  if (sqlite3_step(process_terminal.get()) != SQLITE_ROW) {
    throw HostLedgerError("could not inspect allocation process closure");
  }
  if (sqlite3_column_int(process_terminal.get(), 0) != 0 &&
      sqlite3_column_int(process_terminal.get(), 1) != 0) {
    throw HostLedgerConflict(
        "allocation has no terminal process exit authority");
  }
  const auto request_record = implementation_->make_record(
...
HostProcessLaunchResult SQLiteHostLedger::commit_process_launch_intent(
  ...
  intent.receipt_digest = sha256(
      process_launch_intent_domain(intent.api_version),
...
HostProcessSpawnResult SQLiteHostLedger::commit_process_spawn(
  ...
  receipt.receipt_digest = sha256(
      process_spawn_receipt_domain(receipt.api_version),
...
HostProcessExitResult SQLiteHostLedger::commit_process_exit(
    const HostProcessExitRequest& supplied_request,
    const HostLedgerTime& now) {
  const HostProcessExitRequest request =
      seal_host_process_exit_request(supplied_request);
  if (request != supplied_request || !valid_time(now)) {
    throw HostLedgerError("process exit input is not canonical");
  }
  std::scoped_lock lock(implementation_->mutex);
  Transaction transaction(implementation_->database);
  std::string reason;
  if (!implementation_->verify_unlocked(&reason)) {
    throw HostLedgerError("refusing process exit receipt: " + reason);
  }
  Statement recovered_terminal(implementation_->database, R"sql(
    SELECT 1 FROM process_recovery_exits WHERE launch_id=?
  )sql");
  bind_text(recovered_terminal.get(), 1, request.launch_id);
  if (sqlite3_step(recovered_terminal.get()) == SQLITE_ROW) {
    throw HostLedgerConflict(
        "process already has recovery terminal evidence");
  }
  Statement replay(implementation_->database, R"sql(
    SELECT launch_id, request_digest, canonical_receipt_json
    FROM process_exits
    WHERE launch_id=? OR exit_request_id=?
  )sql");
  bind_text(replay.get(), 1, request.launch_id);
  bind_text(replay.get(), 2, request.exit_request_id);
  if (sqlite3_step(replay.get()) == SQLITE_ROW) {
    if (column_text(replay.get(), 0) != request.launch_id ||
        column_text(replay.get(), 1) != request.canonical_request_digest) {
      throw HostLedgerConflict(
          "process exit identity was reused with different evidence");
    }
    auto receipt = host_process_exit_receipt_from_json(
        nlohmann::json::parse(column_text(replay.get(), 2)));
    transaction.commit();
    return {.receipt = std::move(receipt), .replayed = true};
  }
  Statement spawn_query(implementation_->database, R"sql(
    SELECT spawn.canonical_receipt_json, allocation.status
    FROM process_spawns AS spawn
    JOIN process_launch_intents AS intent USING(launch_id)
    JOIN allocations AS allocation ON allocation.allocation_id=intent.allocation_id
    WHERE spawn.launch_id=?
  )sql");
  bind_text(spawn_query.get(), 1, request.launch_id);
  if (sqlite3_step(spawn_query.get()) != SQLITE_ROW ||
      column_text(spawn_query.get(), 1) != "active") {
    throw HostLedgerConflict(
        "process exit has no spawned child on an active allocation");
  }
  const HostProcessSpawnReceipt spawn = host_process_spawn_receipt_from_json(
      nlohmann::json::parse(column_text(spawn_query.get(), 0)));
  if (request.spawn_receipt_digest != spawn.receipt_digest ||
      request.host_pid != spawn.request.host_pid ||
      request.process_starttime_ticks !=
          spawn.request.process_starttime_ticks ||
      request.cgroup_path != spawn.request.cgroup_path ||
      request.cgroup_device != spawn.request.cgroup_device ||
      request.cgroup_inode != spawn.request.cgroup_inode ||
      spawn.request.boot_id != implementation_->inventory.boot_id) {
    throw HostLedgerConflict(
        "process exit evidence diverges from its exact spawn receipt");
  }
  HostProcessExitReceipt receipt{
      .api_version = std::string(kHostProcessExitReceiptApiVersion),
      .request = request,
      .host_id = implementation_->inventory.host_id,
      .boot_id = implementation_->inventory.boot_id,
      .broker_epoch = implementation_->inventory.broker_epoch,
  ...
  receipt.receipt_digest = sha256(
      ...
      process_exit_receipt_digest_json(receipt).dump());
  const auto record = implementation_->make_process_record(
      "process.exited", "process-exit:" + request.exit_request_id,
      request.launch_id, host_process_exit_receipt_json(receipt));
  const auto appended = implementation_->append_process_record(record);
  implementation_->fault(HostLedgerFaultPoint::after_process_exit_record);
  Statement insert(implementation_->database, R"sql(
    INSERT INTO process_exits(
      launch_id, exit_request_id, request_digest, spawn_receipt_digest,
      receipt_digest, canonical_request_json, canonical_receipt_json,
      record_sequence, record_receipt_digest, record_chain_hash
    ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  )sql");
  bind_text(insert.get(), 1, request.launch_id);
  bind_text(insert.get(), 2, request.exit_request_id);
  bind_text(insert.get(), 3, request.canonical_request_digest);
  bind_text(insert.get(), 4, request.spawn_receipt_digest);
  bind_text(insert.get(), 5, receipt.receipt_digest);
  bind_text(insert.get(), 6, host_process_exit_request_json(request).dump());
  bind_text(insert.get(), 7, host_process_exit_receipt_json(receipt).dump());
  bind_integer(insert.get(), 8, checked_integer(appended.sequence,
                                                "process record sequence"));
  bind_text(insert.get(), 9, appended.receipt_digest);
  bind_text(insert.get(), 10, appended.chain_hash);
  require_done(implementation_->database, insert.get(),
               "process exit projection insert failed");
  implementation_->fault(HostLedgerFaultPoint::after_process_exit_projection);
  if (!implementation_->verify_unlocked(&reason)) {
    throw HostLedgerError("new process exit receipt failed verification: " +
                          reason);
  }
  transaction.commit();
  implementation_->fault(HostLedgerFaultPoint::after_process_exit_commit);
  return {.receipt = std::move(receipt), .replayed = false};
}

HostProcessRecoveryExitResult SQLiteHostLedger::commit_process_recovery_exit(
    const HostProcessRecoveryExitRequest& supplied_request,
    const HostLedgerTime& now) {
  const HostProcessRecoveryExitRequest request =
      seal_host_process_recovery_exit_request(supplied_request);
  if (request != supplied_request || !valid_time(now)) {
    throw HostLedgerError("process recovery exit input is not canonical");
  }
  std::scoped_lock lock(implementation_->mutex);
  Transaction transaction(implementation_->database);
  std::string reason;
  if (!implementation_->verify_unlocked(&reason)) {
    throw HostLedgerError("refusing process recovery exit receipt: " + reason);
  }
  Statement ordinary_terminal(implementation_->database, R"sql(
    SELECT 1 FROM process_exits WHERE launch_id=?
  )sql");
  bind_text(ordinary_terminal.get(), 1, request.launch_id);
  if (sqlite3_step(ordinary_terminal.get()) == SQLITE_ROW) {
    throw HostLedgerConflict("process already has child-wait terminal evidence");
  }
  Statement replay(implementation_->database, R"sql(
    SELECT launch_id, request_digest, canonical_receipt_json
    FROM process_recovery_exits
    WHERE launch_id=? OR recovery_exit_request_id=?
  )sql");
  bind_text(replay.get(), 1, request.launch_id);
  bind_text(replay.get(), 2, request.recovery_exit_request_id);
  if (sqlite3_step(replay.get()) == SQLITE_ROW) {
    if (column_text(replay.get(), 0) != request.launch_id ||
        column_text(replay.get(), 1) != request.canonical_request_digest) {
      throw HostLedgerConflict(
          "process recovery exit identity was reused with different evidence");
    }
    auto receipt = host_process_recovery_exit_receipt_from_json(
        nlohmann::json::parse(column_text(replay.get(), 2)));
    transaction.commit();
    return {.receipt = std::move(receipt), .replayed = true};
  }
  ...
  if (sqlite3_step(spawn_query.get()) != SQLITE_ROW ||
      column_text(spawn_query.get(), 1) != "active") {
    throw HostLedgerConflict(
        "process recovery exit has no spawn on an active allocation");
  }
  const HostProcessSpawnReceipt spawn = host_process_spawn_receipt_from_json(
      nlohmann::json::parse(column_text(spawn_query.get(), 0)));
  if (request.spawn_receipt_digest != spawn.receipt_digest ||
      ...
      spawn.request.boot_id != implementation_->inventory.boot_id) {
    throw HostLedgerConflict(
        "process recovery exit diverges from its exact spawn receipt");
  }
  HostProcessRecoveryExitReceipt receipt{
      .api_version = std::string(kHostProcessRecoveryExitReceiptApiVersion),
      .request = request,
      .host_id = implementation_->inventory.host_id,
      .boot_id = implementation_->inventory.boot_id,
      .broker_epoch = implementation_->inventory.broker_epoch,
  ...
  receipt.receipt_digest = sha256(
      ...
      process_recovery_exit_receipt_digest_json(receipt).dump());
  const auto record = implementation_->make_process_record(
      "process.exited",
      "process-recovery-exit:" + request.recovery_exit_request_id,
      request.launch_id, host_process_recovery_exit_receipt_json(receipt));
  const auto appended = implementation_->append_process_record(record);
  implementation_->fault(
      HostLedgerFaultPoint::after_process_recovery_exit_record);
  Statement insert(implementation_->database, R"sql(
    INSERT INTO process_recovery_exits(
      launch_id, recovery_exit_request_id, request_digest,
      spawn_receipt_digest, receipt_digest, canonical_request_json,
      canonical_receipt_json, record_sequence, record_receipt_digest,
      record_chain_hash
    ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  )sql");
  bind_text(insert.get(), 1, request.launch_id);
  bind_text(insert.get(), 2, request.recovery_exit_request_id);
  bind_text(insert.get(), 3, request.canonical_request_digest);
  bind_text(insert.get(), 4, request.spawn_receipt_digest);
  bind_text(insert.get(), 5, receipt.receipt_digest);
  bind_text(insert.get(), 6,
            host_process_recovery_exit_request_json(request).dump());
  bind_text(insert.get(), 7,
            host_process_recovery_exit_receipt_json(receipt).dump());
  bind_integer(insert.get(), 8, checked_integer(appended.sequence,
                                                "process record sequence"));
  bind_text(insert.get(), 9, appended.receipt_digest);
  bind_text(insert.get(), 10, appended.chain_hash);
  require_done(implementation_->database, insert.get(),
               "process recovery exit projection insert failed");
  implementation_->fault(
      HostLedgerFaultPoint::after_process_recovery_exit_projection);
  if (!implementation_->verify_unlocked(&reason)) {
    throw HostLedgerError(
        "new process recovery exit receipt failed verification: " + reason);
  }
  transaction.commit();
  implementation_->fault(
...
SQLiteHostLedger::active_process_recovery_records(
    std::size_t maximum_records) const {
  if (maximum_records == 0U ||
      maximum_records > HostResourceBounds::maximum_active_fences) {
    throw HostLedgerError("process recovery record bound is invalid");
  }
  std::scoped_lock lock(implementation_->mutex);
  std::string reason;
  if (!implementation_->verify_unlocked(&reason))
    throw HostLedgerError("refusing process recovery read: " + reason);
  Statement records(implementation_->database, R"sql(
    SELECT outcome.canonical_outcome_json, intent.canonical_intent_json,
           spawn.canonical_receipt_json
    FROM process_launch_intents AS intent
    JOIN allocations AS allocation
      ON allocation.allocation_id=intent.allocation_id
    JOIN request_outcomes AS outcome
      ON outcome.request_id=allocation.request_id
    LEFT JOIN process_spawns AS spawn USING(launch_id)
    LEFT JOIN process_exits AS terminal USING(launch_id)
    LEFT JOIN process_recovery_exits AS recovered USING(launch_id)
    WHERE allocation.status='active' AND terminal.launch_id IS NULL AND
          recovered.launch_id IS NULL
  ...
  bind_integer(records.get(), 1,
               checked_integer(maximum_records + 1U,
                               "process recovery record limit"));
  std::vector<HostProcessRecoveryRecord> result;
  int status = SQLITE_ROW;
  while ((status = sqlite3_step(records.get())) == SQLITE_ROW) {
    if (result.size() == maximum_records)
      throw HostLedgerError("process recovery records exceed their bound");
    ...
    if (sqlite3_column_type(records.get(), 2) != SQLITE_NULL) {
      spawn = host_process_spawn_receipt_from_json(
    ...
    if (intent.request.allocation_id != grant.allocation_id ||
      ...
      throw HostLedgerError("process recovery evidence binding is inconsistent");
    }
  ...
  if (status != SQLITE_DONE)
    throw HostLedgerError("process recovery record query failed");
  return result;
}
...
std::string host_device_policy_installation_digest(
  ...
  if (allocation_id.empty() || launch_id.empty() || cgroup_path.empty() ||
      ...
      cgroup_inode == 0U ||
      !valid_device_policy_installation_binding(installation)) {
    throw HostLedgerError(
        "device policy installation digest input is malformed");
  }
  return sha256(
      "trainvm.linux-device-policy-installation/v1",
      nlohmann::json{
          {"api_version", "trainvm.linux-device-policy-installation/v1"},
          ...
          {"cgroup",
           {{"unified_path", cgroup_path},
            ...
            {"inode", cgroup_inode}}},
          {"program",
           {{"program_id", installation.program_id},
            {"program_type", installation.program_type},
            {"program_tag", installation.program_tag},
            {"program_name", installation.program_name}}},
...
HostProcessLaunchRequest seal_host_process_launch_request(
  ...
  request.canonical_request_digest = sha256(
      process_launch_request_domain(request.api_version),
...
nlohmann::json host_process_launch_intent_json(
  ...
  const bool legacy =
      intent.api_version == kHostProcessLaunchIntentApiVersion &&
      intent.request.api_version == kHostProcessLaunchRequestApiVersion;
  const bool device_bound =
  ...
  if ((!legacy && !device_bound) ||
      ...
      intent.receipt_digest !=
          sha256(process_launch_intent_domain(intent.api_version),
                 process_launch_intent_digest_json(intent).dump())) {
...
HostProcessSpawnRequest seal_host_process_spawn_request(
  ...
  request.canonical_request_digest = sha256(
      process_spawn_request_domain(request.api_version),
...
nlohmann::json host_process_spawn_receipt_json(
  ...
  value["receipt_digest"] = receipt.receipt_digest;
  const bool legacy =
      receipt.api_version == kHostProcessSpawnReceiptApiVersion &&
      receipt.request.api_version == kHostProcessSpawnRequestApiVersion;
  const bool device_bound =
  ...
  if ((!legacy && !device_bound) ||
      ...
      receipt.receipt_digest !=
          sha256(process_spawn_receipt_domain(receipt.api_version),
                 process_spawn_receipt_digest_json(receipt).dump())) {
...
HostProcessExitRequest seal_host_process_exit_request(
    HostProcessExitRequest request) {
  if (request.api_version != kHostProcessExitRequestApiVersion ||
      request.exit_request_id.empty() || request.launch_id.empty() ||
      !valid_digest(request.spawn_receipt_digest) || request.host_pid <= 0 ||
      request.process_starttime_ticks == 0U || request.wait_code <= 0 ||
      request.wait_status < 0 || request.cgroup_path.empty() ||
      request.cgroup_path.front() != '/' || request.cgroup_device == 0U ||
      request.cgroup_inode == 0U || !request.cgroup_empty ||
      !request.accelerator_contexts_empty ||
      !valid_digest(request.context_audit_digest)) {
    throw HostLedgerError(
        "process exit request lacks complete terminal evidence");
  }
  request.canonical_request_digest = sha256(
      "trainvm.host-process-exit-request/v1",
      process_exit_request_digest_json(request).dump());
  return request;
}

nlohmann::json host_process_exit_request_json(
    const HostProcessExitRequest& request) {
  if (seal_host_process_exit_request(request) != request) {
    throw HostLedgerError("process exit request digest is not canonical");
  }
  return encode_json(request);
}

HostProcessExitRequest host_process_exit_request_from_json(
    const nlohmann::json& source) {
  auto request = strict_decode<HostProcessExitRequest>(
      source, "process exit request");
  (void)host_process_exit_request_json(request);
  return request;
}

nlohmann::json host_process_exit_receipt_json(
    const HostProcessExitReceipt& receipt) {
  nlohmann::json value = process_exit_receipt_digest_json(receipt);
  value["receipt_digest"] = receipt.receipt_digest;
  if (receipt.api_version != kHostProcessExitReceiptApiVersion ||
      receipt.host_id.empty() || receipt.boot_id.empty() ||
      receipt.broker_epoch.empty() || receipt.observed_boottime_ns < 0 ||
      receipt.observed_wall_time_ns < 0 ||
      !valid_digest(receipt.previous_process_receipt_digest) ||
      receipt.receipt_digest !=
          sha256("trainvm.host-process-exit-receipt/v1",
                 process_exit_receipt_digest_json(receipt).dump())) {
    throw HostLedgerError("process exit receipt is invalid");
  }
  return value;
}

HostProcessExitReceipt host_process_exit_receipt_from_json(
    const nlohmann::json& source) {
  auto receipt = strict_decode<HostProcessExitReceipt>(
      source, "process exit receipt");
  (void)host_process_exit_receipt_json(receipt);
  return receipt;
}

HostProcessRecoveryExitRequest seal_host_process_recovery_exit_request(
    HostProcessRecoveryExitRequest request) {
  if (request.api_version != kHostProcessRecoveryExitRequestApiVersion ||
      request.recovery_exit_request_id.empty() || request.launch_id.empty() ||
      !valid_digest(request.spawn_receipt_digest) || request.host_pid <= 0 ||
      request.process_starttime_ticks == 0U ||
      enum_to_string(request.observation) == "<invalid>" ||
      !valid_digest(request.observation_digest) || request.cgroup_path.empty() ||
      request.cgroup_path.front() != '/' || request.cgroup_device == 0U ||
      request.cgroup_inode == 0U || !request.cgroup_empty ||
      !request.accelerator_contexts_empty ||
      !valid_digest(request.context_audit_digest)) {
    throw HostLedgerError(
        "process recovery exit lacks complete terminal evidence");
  }
  request.canonical_request_digest = sha256(
      "trainvm.host-process-recovery-exit-request/v1",
      process_recovery_exit_request_digest_json(request).dump());
  return request;
}

nlohmann::json host_process_recovery_exit_request_json(
    const HostProcessRecoveryExitRequest& request) {
  if (seal_host_process_recovery_exit_request(request) != request) {
    throw HostLedgerError(
        "process recovery exit request digest is not canonical");
  }
  return encode_json(request);
}

HostProcessRecoveryExitRequest host_process_recovery_exit_request_from_json(
    const nlohmann::json& source) {
  auto request = strict_decode<HostProcessRecoveryExitRequest>(
      source, "process recovery exit request");
  (void)host_process_recovery_exit_request_json(request);
  return request;
}

nlohmann::json host_process_recovery_exit_receipt_json(
    const HostProcessRecoveryExitReceipt& receipt) {
  nlohmann::json value = process_recovery_exit_receipt_digest_json(receipt);
  value["receipt_digest"] = receipt.receipt_digest;
  if (receipt.api_version != kHostProcessRecoveryExitReceiptApiVersion ||
      receipt.host_id.empty() || receipt.boot_id.empty() ||
      receipt.broker_epoch.empty() || receipt.observed_boottime_ns < 0 ||
      receipt.observed_wall_time_ns < 0 ||
      !valid_digest(receipt.previous_process_receipt_digest) ||
      receipt.receipt_digest !=
          sha256("trainvm.host-process-recovery-exit-receipt/v1",
                 process_recovery_exit_receipt_digest_json(receipt).dump())) {
    throw HostLedgerError("process recovery exit receipt is invalid");
  }
  return value;
}

HostProcessRecoveryExitReceipt host_process_recovery_exit_receipt_from_json(
    const nlohmann::json& source) {
  auto receipt = strict_decode<HostProcessRecoveryExitReceipt>(
      source, "process recovery exit receipt");
  (void)host_process_recovery_exit_receipt_json(receipt);
  return receipt;
}
