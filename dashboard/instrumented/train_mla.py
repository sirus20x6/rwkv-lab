...
# trainboard: this is a DUPLICATE of src/rwkv_lab/train_mla.py with dashboard
# signal handling. It lives below the repo root, so prepend src/ before package
# imports resolve when run as `python dashboard/instrumented/train_mla.py`.
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "src")))

from rwkv_lab.load_converted import load_converted_model
from rwkv_lab.mla_module import MLAAttention
from rwkv_lab.mutor_module import MuToRHead, mutor_loss
from rwkv_lab.fsp_module import FSPHead, fsp_loss
from rwkv_lab.parallel_heads_module import ParallelHeads, parallel_heads_loss
from rwkv_lab.safe_torch import safe_torch_load
# Engram imports are deferred to main() because engram_ext lives outside this
# package (under /workspace/git/engram/python). Import at call site only when
...
def main() -> None:
    ...
    if cfg.engram_enabled:
        ...
        # Deferred imports — engram_ext is under /workspace/git/engram/python
        import sys as _sys
        _ENGRAM_PY = "/workspace/git/engram/python"
        ...
        from rwkv_lab.engram_integration import install_engram
        from rwkv_lab.load_mla_engram import _apply_engram_patch, _read_patch
    ...
    if engram_modules:
        ...
        from rwkv_lab.engram_integration import collect_host_embedding_params, collect_gpu_engram_params
    ...
    if optimizer_host is not None:
        from rwkv_lab.engram_integration import collect_host_embedding_params as _chp
