...
CACHE = Path("/thearray/downloads/cache/moe-mla")
...
DEDUP_DB = CACHE / "local_porn_image_dedup.sqlite"
DEDUP_MANIFEST = CURATED / "local_porn_unlabeled_dedup.jsonl"
DEDUP_POLICY = CURATED / "local_porn_dedupe_policy.json"
POLL_SECONDS = 20
...
def maintain_dedupe() -> None:
    ...
    if pending:
        ...
        if not DEDUP_POLICY.is_file():
            ...
            return
        unit = "rwkv-local-porn-dedup-finalize"
        if not active(unit):
            start_unit(unit, [str(ROOT / "scripts/finish_local_porn_dedupe.sh")])
...
def maintain_cache() -> None:
    if cache_unit_running():
        return
    local_receipt = CURATED / "local_porn_teacher_shard_000.cache.json"
    if DEDUP_MANIFEST.is_file() and not local_receipt.is_file():
        start_unit("rwkv-vision-cache-local-porn-000",
                   [str(ROOT / "scripts/cache_local_porn_teacher_shard.sh")])
        return
