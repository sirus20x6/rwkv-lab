...
# sampler state, and train on the image-verified i1 + curated Community Gallery mixture.
# The supplied Community Gallery eval split remains strictly held out.
...
if [[ ! -f curated_vision/vision_next_i1_community_gallery.jsonl ]] || \
   [[ ! -f curated_vision/vision_next_i1_community_gallery_eval.jsonl ]]; then
  echo "missing i1/Community Gallery manifests; run: python scripts/build_i1_community_gallery_mix.py" >&2
  exit 1
fi
RUN=runs/moonvit_rwkv_i1_community_gallery_phase2
mkdir -p "$RUN"
...
# Reuse every stage-1 entry and fill only new i1/Community Gallery/eval images. MoonViT's
...
if [[ "${SKIP_CACHE_VERIFY:-0}" != 1 ]]; then
  ...
  for ((shard=0; shard<CACHE_SHARDS; shard++)); do
    PYTHONPATH=src python -m rwkv_lab.vision_cache \
      --data curated_vision/vision_next_i1_community_gallery.jsonl \
             curated_vision/vision_next_i1_community_gallery_eval.jsonl \
...
PYTHONPATH=src python -m rwkv_lab.vision_train \
  --data curated_vision/vision_next_i1_community_gallery.jsonl \
  --eval-data curated_vision/vision_next_i1_community_gallery_eval.jsonl \
  ...
  --max-text-tokens 768 \
  --out runs/moonvit_rwkv_i1_community_gallery_phase2 \
