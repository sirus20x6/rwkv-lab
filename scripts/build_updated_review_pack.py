...
def main() -> None:
    ...
    for item in manifest:
        ...
        if dataset in {"restricted_detect", "joy-captioning-20250408a"} or not item.get("image"):
            continue
        image = source / item["image"]
        record = key[item["id"]]
        reference = item["reference"]
        if dataset == "restricted-video-still-caption-grid-only":
        ...
        elif dataset == "restricted_scene_animes_31-03":
            reference = clean_anime_caption((ROOT / record["original"]).read_text().strip())
        elif dataset == "RESTRICTED_Manga":
            reference = "\n".join(
