...
RUN="${RADIO_RUN:-runs/radio1d_rwkv_civitai_joy_continuation}"
...
SOURCE_RUN="${RADIO_SOURCE_RUN:-runs/radio1d_rwkv_captioning_first}"
...
CACHE_MARKER="$CACHE/.civitai-joy-${TRAIN_SHA}-${EVAL_SHA}.complete"
...
for required in "$TRAIN" "$EVAL" "$SOURCE_CHECKPOINT" "$CACHE" "$CACHE_MARKER" \
                models/vision/C-RADIOv4-1D-H \
                models/rwkv7-g1h-2.9b-20260710-ctx10240.pth; do
  [[ -e "$required" ]] || {
    echo "missing Civitai/Joy continuation input: $required" >&2
    exit 1
  }
done
...
while true; do
  ...
  PYTHONPATH=src "$PYTHON_BIN" -m rwkv_lab.vision_train \
    ...
    --out "$RUN" --steps "${RADIO_STEPS:-15000}" \
    --feature-cache "$CACHE" \
    ...
    --eval-examples "${RADIO_EVAL_EXAMPLES:-704}" \
    ...
    --eval-sample-max-new 768 --require-fused-ce --restart-before-eval \
