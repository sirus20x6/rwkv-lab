...
SOURCE_RUN="${VISION_FINISH_SOURCE_RUN:-runs/moonvit_rwkv_i1_community_gallery_phase2}"
...
if int(summary.get("raw_community_gallery_rows", -1)) != 0:
    raise SystemExit("finishing receipt contains raw Community Gallery rows")
...
PYTHONPATH=src "$PYTHON_BIN" -m rwkv_lab.vision_train \
  ...
  --eval-sample-exclude-sources "${VISION_EVAL_SAMPLE_EXCLUDE_SOURCES:-joy,community_gallery,restricted,private_web,manga,pose_vr,grid}" \
