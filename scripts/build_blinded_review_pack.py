...
def main() -> None:
    ...
    grid_root = ROOT / "restricted-video-still-caption-grid-only/extracted"
    grid = choose(sorted(grid_root.glob("*/*.json")), args.per_dataset, rng)
    for meta_path in grid:
        meta = json.loads(meta_path.read_text())
        image = meta_path.with_suffix(".jpg")
        add(source="restricted-video-still-caption-grid-only", original=str(meta_path.relative_to(ROOT)),
    ...
    pose_rows = choose(parquet_rows([ROOT / "pose_vrlens_restricted/data/train-00000-of-00001.parquet"]), args.per_dataset, rng)
    for row in pose_rows:
        ...
        key.append({"id": item_id, "source_dataset": "pose_vrlens_restricted", "original": row["image"].get("path"), "url": None})

    # 3. Anime image/tag-text pairs.  Restrict to B variants because those have captions.
    anime_root = ROOT / "restricted_scene_animes_31-03/extracted"
    anime = choose(sorted(anime_root.glob("*_B.txt")), args.per_dataset, rng)
    for caption in anime:
        image = caption.with_suffix(".jpg")
        add(source="restricted_scene_animes_31-03", original=str(caption.relative_to(ROOT)), copy_from=image,
            task="image_tag_grounding", review_text=caption.read_text().strip())

    # 4. Detector labels, stratified across its five classes.
    detector_root = ROOT / "restricted_detect/extracted/restricted_dataset_v1"
    ...
    for label, image in detector[:args.per_dataset]:
        add(source="restricted_detect", original=str(image.relative_to(ROOT)), copy_from=image, task="safety_label_quality", review_text=label)
    ...
    manga_rows = choose(parquet_rows([ROOT / "RESTRICTED_Manga/manga_all.parquet"]), args.per_dataset, rng)
    for row in manga_rows:
        reference = "\n".join((f"Title: {row['title']}", f"Tags: {row['tags']}", f"Categories: {row['categories']}",
                                f"Language: {row['language']}"))
        add(source="RESTRICTED_Manga", original=str(row["id"]), url=row.get("cover_image"), task="metadata_to_cover_consistency", review_text=reference)
