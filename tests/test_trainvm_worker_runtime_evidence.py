...
def test_device_identity_is_measured_rather_than_taken_from_the_caller():
    ...
    # address is well formed and resolves to no device on any host, so the
    # driver has nothing to say -- and there is no other source to fall back
    # to. Asserted here rather than through the whole probe because a runner
    # with no NVIDIA driver refuses an accelerated launch earlier, which would
    # pass this test for the wrong reason.
    assert _nvidia_device_identity("ffff:ff:ff.f") == {}
    # An unidentifiable device therefore cannot be laundered into a placement
    # claim, whether the host has a driver or not.
    with pytest.raises(RuntimeEvidenceError):
        measure_worker_runtime_evidence(
            _bootstrap(),
