...
# === trainboard instrumentation: dashboard2/instrumented copy of ../../convert_train.py
# (original never modified). Adds ONLY the dashboard's signal hooks — live-tuning is
# already native here via live_controls.LiveControls. Put the repo root on sys.path
# before the repo-local imports so the copy also runs from this subdir. ===
import signal
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)
_SIG = {"ckpt": False, "stop": False}  # SIGUSR1 = checkpoint-now; SIGINT = graceful stop
def _on_sigusr1(_s, _f): _SIG["ckpt"] = True
def _on_sigint(_s, _f): _SIG["stop"] = True
# === end instrumentation header ===
...
def make_optimizer(student, codec, args, text_cfg=None):
    ...
    if args.optimizer == "schedulefree":
        return _make_schedulefree(student, args)
    decay_names = ("w0", "w1", "w2", "a0", "a1", "a2", "k_k", "k_a")
    decay_p, proj_p = [], []
    for n, p in student.named_parameters():
        if not p.requires_grad:
            continue
        (decay_p if any(n.startswith(d) or n == d for d in decay_names) else proj_p).append(p)
    groups = [
...
def _make_schedulefree(student, args):
...
def train(args):
    ...
    if args.fixed_trainset > 0:
        ...
        fixed_starts = [int(pool_rng.integers(0, max_start + 1)) for _ in range(args.fixed_trainset)]
        print(f"[grokking] fixed trainset: cycling {args.fixed_trainset} cached windows "
              f"(reuse-data) -> memorize→generalize regime", flush=True)
    ...
    apilot = GrokAutopilot(student, codec, opt, out, is_sf, enabled=bool(args.grok_autopilot),
                           ...
                           patience=args.ap_patience, max_restarts=args.ap_max_restarts,
    ...
    for step in range(args.steps):
        ...
        if eval_every > 0 and step % eval_every == 0:
            ...
            ev = evaluate(text_model, lm_head, toks, args.eval_windows, args.seq_len, dev,
                          ...
                          box=box if args.log_grokking_metrics else None)
        ...
        with torch.set_grad_enabled(True):
            ...
            block = F.mse_loss(student_out.float(), teacher_out.float())
        ...
        if sched is not None:
            sched.step()
            if lr_scale != 1.0:                       # live LR multiplier (scheduled opts)
                for g in opt.param_groups:
                    g["lr"] = g["lr"] * lr_scale
...
def main():
    ...
    ap.add_argument("--loop-lr-mult", type=float, default=30.0,
                    help="LR multiplier for residual_weight (the loop gates). Its gradient is small, so "
                         "at the base LR the loop barely engages; a higher mult lets it reach useful magnitude.")
    ...
    ap.add_argument("--optimizer", default="adamw", choices=["adamw", "muonclip", "schedulefree"])
    ...
    ap.add_argument("--muon-adam-lr", type=float, default=3e-4,
                    help="AdamW lr for the 1D/scalar/norm params under --optimizer muonclip")
    ...
    ap.add_argument("--control-db",
                    default=str(Path(_REPO_ROOT) / "dashboard2" / "trainboard.db"),
