...
- Keep implementation boundaries aligned with component ownership: optimizer algorithms,
  ...
  objective and BF16/FP32 precision policy behind the stable `training_components` facade;
  category dependency checks prevent regressions. Activation, normalization, distributed
  synchronization/mid-update accumulation, and curriculum modules are created as their first real
  adapter migrations land, not as empty scaffolding.
