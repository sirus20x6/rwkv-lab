...
| Recurrent optimizer A/B harness | the sealed `rwkv-lab.rwkv-optimizer-finetune@1.0.0` adapter runs a fixed-step pretrained BlinkDL RWKV-7 arm with either typed SpectralMuon or AdamW-no-decay, a typed RWKV matrix/fallback router, independent LR/decay/clipping components, live train/eval/system telemetry, bounded profiling, and compatible safe-point checkpoints | content-bound model/token inputs, deterministic validation windows and training cursor, component/routing identity, optimizer global RSAV state, schedule and Python/NumPy/Torch/CUDA RNG are implemented; the legacy wall-clock budget is intentionally not migrated | fixed-step campaign authoring and real-GPU resume qualification remain |
