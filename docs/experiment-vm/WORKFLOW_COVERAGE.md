...
| MageFlow routed/terminal/TREAD expert training | separately reviewed expert plan/cache/train and terminal prepare/cache/cache-span/domain-window/train effects, plus the terminal-expert migration, TREAD conversion, and cache-resume supervisors | `rwkv-lab.mageflow-tread-convert@1.0.0` consumes a verified terminal checkpoint and content-bound local base/config, then publishes a zero-gated controller with exact parent and object-digest receipt. Terminal training accepts it only beside that same checkpoint, treats the handoff as an explicit optimizer-reset architecture migration, and now saves and reloads the complete learned controller in every later checkpoint. Safe-point controls, live metrics, and periodic/terminal generated/original evaluation remain unified | source implementation, closed runtime/authoring contract, checkpoint round-trip, lineage validation, focused CPU tests, and full checkpoint→TREAD/cache→training campaign implemented; current-head native build, explicit standalone eval-node revisions, and real-GPU qualification remain |
...
gRPC already-completed replay, lowers all twenty-four real adapter profiles into the v3 host registry, and
