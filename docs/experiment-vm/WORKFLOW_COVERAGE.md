...
| MageFlow cache boundary and resume | the legacy shell polls `status.json`, `pgrep`, and `jq`, rewrites configs, then launches cache and trainer processes | sealed `rwkv-lab.mageflow-cache-plan@1.0.0` and `rwkv-lab.mageflow-cache-build@1.0.0` operations now verify an immutable compatible-grade checkpoint, publish exact future-example coverage, materialize a restart-only frozen-encoder dataset with per-image graceful-stop safe points, and bind that cache plus its checkpoint as optional typed inputs to terminal MageFlow training; the consumer re-verifies cache/checkpoint ancestry, coverage bytes, referenced image content, and the covered optimizer-step boundary. `examples/mageflow-cache-artifacts.json` demonstrates the complete cross-node handoff, with a separate content-root lock document | implemented plan/cache/checkpoint artifact lineage, cross-node continuation, terminal consumption, source-level tests, and declarative example; native build and real-GPU cache→resume trajectory qualification remain |
...
| MageFlow routed/terminal/TREAD expert training | separately reviewed expert plan/cache/train and terminal prepare/cache/cache-span/domain-window/train effects, plus the terminal-expert migration, TREAD conversion, and cache-resume supervisors | the terminal trainer, immutable cache-span handoff, component freeze map, safe-point controls, checkpoint publication, live metrics, and periodic/terminal unified evaluation are implemented; generated/original balanced route-aware gallery items publish only after the fixed worker freezes their same-step checkpoint | implemented route/component/checkpoint/cache state plus intermediate and terminal unified galleries; TREAD conversion publication, full campaign authoring, explicit standalone eval-node revisions, and real-GPU qualification remain |
...
gRPC already-completed replay, lowers all twenty-three real adapter profiles into the v3 host registry, and
