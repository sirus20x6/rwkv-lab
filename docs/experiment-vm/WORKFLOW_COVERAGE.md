...
| vision representation A/B | `examples/vision-representation-ab.json` replaces the legacy Python/shell supervisor with two sequential, node-isolated `rwkv-lab.vision-frozen-adapter` operations under one exclusive lease; the compiler and production adapter registry validate the graph, and each arm publishes its own immutable compatible checkpoint | freeze the listed static data/model/cache roots before submission; add a versioned cross-arm metric/result and promotion decision only when campaign-level automatic selection is desired | legacy launch parity implemented; content-lock authoring convenience and optional automated decision receipt remain |
...
gRPC already-completed replay, lowers all eighteen real adapter profiles into the v3 host registry, and
