...
| MageFlow full-backbone continued pretraining | canonical `mage_flow_pretrain` now backs the sealed `rwkv-lab.mageflow-full-backbone@1.0.0` profile; preparation remains a separate compatibility surface | one compatible-resume full NR-MMDiT operation content-binds the local model tree, train/eval manifests and every referenced image; consumes authority-selected AdamW-no-decay, LR, decay, clipping and exhaustive full-backbone routing; emits live train/eval/throughput/memory telemetry; supports safe-point controls and bounded compile/warmup; publishes Accelerate model/optimizer/RNG plus epoch/pack state; and returns a same-step generated/original gallery request that the fixed worker binds to the newly sealed checkpoint | implemented training and immutable terminal gallery publication; exact trajectory qualification, append-only intermediate eval revisions, and typed preparation artifact publication remain |
| MageFlow routed/terminal/TREAD expert training | separately reviewed expert plan/cache/train and terminal prepare/cache/cache-span/domain-window/train effects, plus the terminal-expert migration, TREAD conversion, and cache-resume supervisors | route/domain-window graph with cache-span handoffs, component freeze map, migration and joint-eval nodes; terminal completion guarantees balanced route-aware generated/original items and returns a typed gallery request bound by the fixed runner to its immutable checkpoint | route state, domain cursor, cache coverage, expert/base checkpoints, and terminal unified gallery implemented; intermediate checkpoint/eval-node revisions and real GPU qualification remain |
...
gRPC already-completed replay, lowers all twenty-two real adapter profiles into the v3 host registry, and
