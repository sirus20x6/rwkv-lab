...
| fixed-step pretrained RWKV optimizer finetuning | `compatible` | `rwkv-lab.rwkv-optimizer-finetune@1.0.0` content-binds the model and uint16 token stream, routes matrix/fallback parameters explicitly, restores model/optimizer/schedule/data-cursor/RNG/control state, and publishes safe-point checkpoints; the legacy wall-clock `rwkv_finetune` surface remains unsupported |
| consolidation and current RWKV post-training arms | `restart_only` | interruption restarts a bounded operation from immutable inputs |
...
| canonical `vision_train` | `compatible` | the sealed `rwkv-lab.vision-frozen-adapter@1.0.0` profile exposes the MoonViT and compressor arms as typed configuration; content-binds checkpoints, vocabulary, manifests/image references, read-only caches, and compressor model trees; restores sampler plus Python/Torch/CUDA RNG and control state; consumes typed AdamW/clipping/precision; and publishes required checkpoints. Exact trajectory equivalence remains unproven |
...
| vision native head | `compatible` | the canonical trainer now backs sealed `rwkv-lab.vision-native-head@1.0.0`: baseline/compressor checkpoints, manifests, caches, teacher/model identities, and vocabulary are content-bound; optimizer/schedule/decay/clipping/precision composition, live telemetry/profiling, controller resume, and required checkpoint publication are implemented. The length-bucket sampler cursor is not checkpointed, so exactness is not claimed |
| vision raw-pixel student | `compatible` | the sealed `rwkv-lab.vision-rwkv-student@1.0.0` profile content-binds the baseline, compressor, native head, vocabulary, manifests, caches, and teacher/model trees; restores the exact sampler cursor plus Python/Torch/CUDA RNG; consumes typed optimizer/schedule/decay/clipping/precision components; emits live train/eval telemetry and traces; and publishes a required checkpoint. Exact trajectory equivalence remains unproven |
...
| Recurrent optimizer A/B harness | the sealed `rwkv-lab.rwkv-optimizer-finetune@1.0.0` adapter runs a fixed-step pretrained BlinkDL RWKV-7 arm with either typed SpectralMuon or AdamW-no-decay, a typed RWKV matrix/fallback router, independent LR/decay/clipping components, live train/eval/system telemetry, bounded profiling, compatible safe-point checkpoints, and immutable final-loss reports | `examples/rwkv-optimizer-ab.json` runs deterministic sequential arms and an authority-owned scalar decision over content-bound model/token inputs; component/routing identity, optimizer global RSAV state, schedule and Python/NumPy/Torch/CUDA RNG are implemented; the legacy wall-clock budget is intentionally not migrated | real-GPU resume and trajectory qualification remain |
...
| Frozen-vision-to-LM adapter training | canonical `vision_train` now backs the sealed `rwkv-lab.vision-frozen-adapter@1.0.0` profile | compatible-resume MoonViT/compressor train/eval nodes consume immutable prebuilt caches, typed training components, safe-point controls, live caption/grounding metrics and traces, and required checkpoints | implemented input/cache closure, deterministic state, and protocol publication; exact-resume golden qualification remains |
| Native vision head and raw-pixel student training | canonical trainers now back separate sealed `rwkv-lab.vision-native-head@1.0.0` and `rwkv-lab.vision-rwkv-student@1.0.0` profiles | two compatible-grade operations content-bind every baseline/compressor/native-head/manifest/cache/teacher/model/vocabulary input, enforce distinct trainable scopes, consume typed components, emit transfer metrics/traces, and publish required checkpoints | implemented input closure and honest compatible resume; native-head exact cursor recovery and both golden trajectory gates remain |
| MageFlow cache boundary and resume | the legacy shell polls `status.json`, `pgrep`, and `jq`, rewrites configs, then launches cache and trainer processes | sealed `rwkv-lab.mageflow-cache-plan@1.0.0` and `rwkv-lab.mageflow-cache-build@1.0.0` operations verify one immutable compatible-grade checkpoint, publish exact future-example coverage, and materialize a restart-only frozen-encoder dataset. The terminal consumer re-verifies cache/checkpoint ancestry, coverage bytes, referenced image content, and the covered optimizer-step boundary. `examples/mageflow-cache-artifacts.json` demonstrates the complete checkpoint→TREAD/cache→training handoff, with a separate content-root lock document | implemented plan/cache/checkpoint/TREAD artifact lineage, cross-node continuation, terminal consumption, focused source-level tests, and declarative campaign; current-head native build and real-GPU cache→resume trajectory qualification remain |
| MageFlow full-backbone continued pretraining | canonical `mage_flow_pretrain` now backs the sealed `rwkv-lab.mageflow-full-backbone@1.0.0` profile; preparation remains a separate compatibility surface | one compatible-resume full NR-MMDiT operation content-binds the local model tree, train/eval manifests and every referenced image; consumes authority-selected AdamW-no-decay, LR, decay, clipping and exhaustive full-backbone routing; emits live train/eval/throughput/memory telemetry; supports safe-point controls and bounded compile/warmup; publishes Accelerate model/optimizer/RNG plus epoch/pack state; and streams same-step checkpoint-bound generated/original gallery revisions through fixed-worker authority | implemented training plus immutable intermediate and terminal gallery publication; exact trajectory qualification and typed preparation artifact publication remain |
| MageFlow routed/terminal/TREAD expert training | separately reviewed expert plan/cache/train and terminal prepare/cache/cache-span/domain-window/train effects, plus the terminal-expert migration, TREAD conversion, cache-resume supervisors, and immutable checkpoint evaluation | `rwkv-lab.mageflow-tread-convert@1.0.0` consumes a verified terminal checkpoint and content-bound local base/config, then publishes a zero-gated controller with exact parent and object-digest receipt. Terminal training accepts it only beside that same checkpoint, treats the handoff as an explicit optimizer-reset architecture migration, and saves and reloads the complete learned controller in every later checkpoint. `rwkv-lab.mageflow-eval@1.0.0` reconstructs the embedded terminal architecture without an optimizer or replacement checkpoint, then publishes finite metrics and a generated/original gallery revision directly bound to the selected checkpoint manifest | source implementation, closed runtime/authoring contracts, checkpoint round-trip and eval-lineage validation, focused CPU tests, and full checkpoint→TREAD/cache→training→evaluation campaign implemented; current-head native build and real-GPU qualification remain |
...
| vision representation A/B | `examples/vision-representation-ab.json` replaces the legacy Python/shell supervisor with two sequential, node-isolated `rwkv-lab.vision-frozen-adapter` operations under one exclusive lease; each arm publishes a compatible checkpoint and immutable `eval.loss` result, then the stateless `rwkv-lab.scalar-metric-decision` operation independently verifies both artifact trees and publishes the explicit minimize/tolerance decision with parent lineage | `vision-representation-ab.input-roots.json` plus native `trainvm lock-input-content` measure and inject the static data/model/cache identities before submission; reuse the generic scalar result/decision contract for later campaigns | run arms, content-lock authoring, versioned results, deterministic decision, and compiler/registry validation implemented; real GPU qualification remains |
...
| Typed native RWKV vision-head calibration | canonical `vision_native_train` now backs the sealed `rwkv-lab.vision-native-head@1.0.0` profile | one compatible-resume node freezes the caption stack and teacher compressor, trains only the compressor-owned native output head, emits loss/PPL/throughput/gradient telemetry and traces, and publishes a required checkpoint | implemented input lineage and trainable-scope enforcement; exact sampler cursor remains |
| Typed raw-pixel Vision-RWKV student distillation | canonical `vision_rwkv_student_train` now backs the sealed `rwkv-lab.vision-rwkv-student@1.0.0` profile | one compatible-resume node freezes the caption and six-stream teacher stack, trains only the deployable raw-pixel student, restores an exact shuffled sampler cursor and RNG state, emits representation/caption/PPL/throughput/gradient telemetry and bounded traces, and publishes a required checkpoint | implemented full input lineage, trainable-scope enforcement, component/control identity, and deterministic checkpoint state; exact-resume golden qualification remains |
| Typed vision teacher compression | canonical `vision_teacher_compressor` now backs the sealed `rwkv-lab.vision-teacher-compressor@1.0.0` profile | one resumable vision-distillation node resolves every manifest/cache/model path through content authority, consumes a closed constant-LR/AdamW/FP32-parameter-BF16-compute component composition, emits train/eval metrics and bounded traces, and protocol-publishes a required compatible checkpoint | implemented deterministic data cursor and Python/Torch/CUDA RNG recovery; exact-resume golden qualification remains |
...
| Single-run RWKV RLVR | canonical `rlvr_train` now backs the sealed `rwkv-lab.rwkv-rlvr@1.0.0` profile for GSPO, Dr.GRPO, or DAPO; checkpoint, vocabulary, optional task splits, reference checkpoint, and direct-argv verifier executable are content-bound | one bounded terminal-checkpoint candidate consumes authority-selected AdamW-no-decay, linear-warmup/constant LR, weight-decay, and clipping components; emits live reward/pass@k/loss/KL/LR/gradient telemetry plus bounded traces; and publishes an immutable candidate checkpoint with promotion evidence. Legacy `--resume`, 8-bit optimizers, and safe-point checkpoint/pause remain migration work and are not overclaimed | implemented typed budgets, held-out promotion gates, verifier identity, component evidence, terminal checkpoint publication; upgrade to compatible resume only after config/task/verifier/reference identity and CUDA RNG are verified on recovery |
...
| Typed RWKV adapter post-training | canonical `posttrain_train` now backs the exact `rwkv-lab.rwkv-posttraining@1.0.0` TrainVM profile for SFT/DPO/KTO/ORPO/SimPO/RM/PRM over content-bound RWKV checkpoints and datasets | the restart-only objective arm consumes authority-resolved optimizer, LR schedule, weight decay, and clipping components; emits live loss/LR/throughput/eval telemetry and bounded GPU traces; and protocol-publishes one invocation-bound immutable adapter/reward-head/result bundle. Interruption restarts from the immutable parent, and other model families still require their own registered objective adapters | implemented role/reset masks, equal scored-token accounting, immutable final adapter/head and calibration/family report bundle; campaign promotion remains a separate migration |
...
RWKV compatibility coverage is 69 direct, 60 transitive, and 36 uncovered library
or research surfaces, with no runnable direct gap. Native TrainVM covers
`rwkv_lab.registry` campaign listing, latest-result collapse, comparison views,
and shared statistical decision helpers. Its writable consumers are mapped to
declarative campaigns, journal events, and typed result/artifact publication rather than migrated
as another shared database writer. Native runs also expose a bounded, prefix-fenced,
integrity-bound reproducibility capsule through the dashboard. This mapping grants no permission to
launch the legacy Python module or let a worker share a writable database.
...
gRPC already-completed replay, lowers all twenty-five real adapter profiles into the v3 host registry, and
