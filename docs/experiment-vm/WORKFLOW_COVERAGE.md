...
| MageFlow routed/terminal/TREAD expert training | separately reviewed expert plan/cache/train and terminal prepare/cache/cache-span/domain-window/train effects, plus the terminal-expert migration, TREAD conversion, and cache-resume supervisors | route/domain-window graph with cache-span handoffs, component freeze map, migration and joint-eval nodes; periodic and terminal unified evaluations publish balanced route-aware generated/original items only after the fixed worker freezes their same-step checkpoint | route state, domain cursor, cache coverage, expert/base checkpoints, and intermediate/terminal unified galleries implemented; explicit standalone eval-node revisions and real GPU qualification remain |
...
gRPC already-completed replay, lowers all twenty-one real adapter profiles into the v3 host registry, and
