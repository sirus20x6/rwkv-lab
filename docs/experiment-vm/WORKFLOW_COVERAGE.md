...
| vision representation A/B | `examples/vision-representation-ab.json` replaces the legacy Python/shell supervisor with two sequential, node-isolated `rwkv-lab.vision-frozen-adapter` operations under one exclusive lease; each arm publishes a compatible checkpoint and immutable `eval.loss` result, then the stateless `rwkv-lab.scalar-metric-decision` operation independently verifies both artifact trees and publishes the explicit minimize/tolerance decision with parent lineage | freeze the listed static data/model/cache roots before submission; reuse the generic scalar result/decision contract for later campaigns | run arms, versioned results, deterministic decision, compiler/registry validation implemented; content-lock authoring convenience remains |
