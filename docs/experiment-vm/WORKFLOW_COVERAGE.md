...
| vision raw-pixel student | `compatible` | missing cursor or external-input identity prevents exactness |
...
| Typed native RWKV vision-head calibration | canonical `vision_native_train` now backs the sealed `rwkv-lab.vision-native-head@1.0.0` profile | one compatible-resume node freezes the caption stack and teacher compressor, trains only the compressor-owned native output head, emits loss/PPL/throughput/gradient telemetry and traces, and publishes a required checkpoint | implemented input lineage and trainable-scope enforcement; exact sampler cursor and raw-pixel student handoff remain |
...
gRPC already-completed replay, lowers all sixteen real adapter profiles into the v3 host registry, and
